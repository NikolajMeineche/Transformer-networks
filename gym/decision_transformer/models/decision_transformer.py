...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.embed_return1 = torch.nn.Linear(1, hidden_size)
        self.embed_return2 = torch.nn.Linear(1, hidden_size)
        ...
        #self.predict_return = torch.nn.Linear(hidden_size, 1) #bjarke overridden
    ...
    def forward(self, states, actions, r1,r2, returns_to_go1,returns_to_go2, timesteps, attention_mask=None):
        ...
        returns_embeddings1 = returns_embeddings1 + time_embeddings #bjarke
        returns_embeddings2 = returns_embeddings2 + time_embeddings #bjarke
        # this makes the sequence look like (R_1, s_1, a_1, R_2, s_2, a_2, ...)
        # which works nice in an autoregressive sense since states predict actions
        stacked_inputs = torch.stack(
            (returns_embeddings1, returns_embeddings2, state_embeddings, action_embeddings), dim=1
        ).permute(0, 2, 1, 4).reshape(batch_size, 3*seq_length, self.hidden_size) #Bjarke ændrede 3 til 4
        ...
        #Bjarke: return preds should return 2
        return_preds1 = self.predict_return1(x[:, 2]) #bjarke
        return_preds2 = self.predict_return2(x[:, 2]) #bjarke
